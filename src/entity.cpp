...
namespace mnes {
    ...
    void EntityManager::AddComponent(Entity e, Component* c) {
        ...
        if (HasComponent(e, typeid(*c)))        { std::cout << "Entity '" << e_id << "' already contains component '" << typeid(*c).name() << "'."; return; }
    ...
    void EntityManager::RemoveComponent(Entity e, std::type_index t) {
        ...
        int e_id = e.GetId();
        unsigned int bit = 1 << c_id;        
        if (!HasComponent(e, t)) { std::cout << "Can't remove component '" << t.name() << "' from entity '" << e_id << "' as it does not contain that component"; return; }
    ...
    Component* EntityManager::GetComponent(Entity e, std::type_index t) {
        int c_id = Components::GetId(t);
        int e_id = e.GetId();
        if (!HasComponent(e, t)) { std::cout << "Entity '" << e_id << "' does not contain component '" << t.name() << "'."; return nullptr; }
